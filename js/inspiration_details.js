        ...
        success: function(str) { //返回json结果
            if (str.status == 200) {
                ...
                if (author_id == get_user_info("user_userid")) {
                    $("#edit").attr("href", "inspiration_edit.html?id=" + inspiration_id);
                    $("#edit").show();
...
// 教程分页内容
function get_inspirations_comment_paging(id, pagenum) {
    var fid = id; // 文章id
    var ctype = 2; // 0教程 1提问 2灵感 3文章
        ...
        success: function(str) { //返回json结果
            if (str.status == 200) {
                ...
                for (var i = 0; i < datas.length; i++) {
                    ...
                    var inspiration_comment = datas[i].comment; // 简介
                    var inspiration_creattime = datas[i].times;
                    ...
                    var c = '<div class="comment-item">' +
                        ...
                        '</div>' +
                        '<div class="date">' + inspiration_creattime + '</div>' +
                        '<p class="word" style="word-break:break-all;">' + inspiration_comment + '</p>' +
                ...
                compute_pagenum(pagenum, "get_inspirations_comment_paging", $('#inspiration_id').val())
                    // 已点赞和已收藏的用户显示红色点赞和红色收藏按钮，待做
            } else {
                alert("获取数据失败！");
                remove_user_login_status(str.msg)
